...
// Denoising
void FastNlMeansDenoisingColoredMulti( struct Mats src, Mat dst, int imgToDenoiseIndex, int 	temporalWindowSize, float 	h, float 	hColor, int 	templateWindowSize, int 	searchWindowSize ){
  std::vector<cv::Mat> images;
  for (int i = 0; i < src.length; ++i) {
    images.push_back(*src.mats[i]); // Key trick here : *src.mats is an array of several cv::Mat
  }
...
// struct Mats is a goCV inherent structure which is a pointer on a series of Mat that deals with a vector of Mats
void MergeMertens_Process(MergeMertens b, struct Mats src, Mat dst) {
  std::vector<cv::Mat> images;
  for (int i = 0; i < src.length; ++i) {
    images.push_back(*src.mats[i]); // Key trick here : *src.mats is an array of several cv::Mat
  }
  (*b)->process(images, *dst); // Native OpenCV C++ instruction which is a call from MergeMertens::process
}
